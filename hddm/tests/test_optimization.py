...
def optimization_recovery_single_subject(
    repeats=10, seed=1, true_starting_point=True, optimization_method="ML"
):
    """
    ...
    # init
    include_sets = [set(["a", "v", "t"]), set(["a", "v", "t", "z"])]
    ...
    for include in include_sets:
        for i in range(repeats):
            ...
            h = hddm.models.HDDMTruncated(
                samples, include=include, depends_on={"v": "condition"}
            )
            ...
            if optimization_method == "ML":
                ...
                recovered_params = h.optimize(
                    method="chisquare", quantiles=np.linspace(0.05, 0.95, 10)
                )
            elif optimization_method == "gsquare":
                recovered_params = h.optimize(
                    method="gsquare", quantiles=np.linspace(0.05, 0.95, 10)
                )
            else:
...
def test_ML_recovery_single_subject_from_random_starting_point():
    optimization_recovery_single_subject(
        repeats=5, seed=1, true_starting_point=False, optimization_method="ML"
    )


def test_ML_recovery_single_subject_from_true_starting_point():
    optimization_recovery_single_subject(
        repeats=5, seed=1, true_starting_point=True, optimization_method="ML"
    )


def test_chisquare_recovery_single_subject_from_true_starting_point():
    optimization_recovery_single_subject(
        repeats=5, seed=1, true_starting_point=True, optimization_method="chisquare"
    )


def test_gsquare_recovery_single_subject_from_true_starting_point():
    optimization_recovery_single_subject(
        repeats=5, seed=1, true_starting_point=True, optimization_method="gsquare"
    )
    ...
    for include in include_sets:
        for i in range(repeats):
            ...
            if random_p_outlier is None:
                ...
                h = hddm.models.HDDM(
                    samples,
                    include=include.union(["p_outlier"]),
                    depends_on={"v": "condition"},
                )
            else:
                h = hddm.models.HDDM(
